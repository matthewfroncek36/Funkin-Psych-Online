...
class CreateMacroAbstractClass {
	//just allow used things, otherwise compiling time is 💀
	...
	public static function init() {
		...
		for(apply in ALLOWED_ABSTRACT_AND_ENUM) {
			Compiler.addGlobalMetadata(apply, '@:build(macros.CreateMacroAbstractClass.build())');
		}
		#end
	}
	...
	public static function build():Array<Field> {
		...
		if (/* cl.name.startsWith("Flx") && */ cl.name.endsWith("_Impl_") && cl.params.length <= 0 && !cl.meta.has(":multiType") && !cl.name.contains("_HSC")) {
			...
			if(DISALLOW_ABSTRACT_AND_ENUM.contains(cl.module) || DISALLOW_ABSTRACT_AND_ENUM.contains(fkey)) return fields;
