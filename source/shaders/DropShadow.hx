...
{
	/**
	 * The drop shadow shader. Use this value to actually set the shader.
	 */
	public var shader(default, null):DropShadowShader;
	...
	{
		color = col;
		shader.dropColor.value = [color.red / 255, color.green / 255, color.blue / 255];
	...
	{
		angle = val;
		shader.ang.value = [angle * FlxAngle.TO_RAD];
		return angle;
	}
	 ...
	 * While you *could* directly set the value of the mask, this function uses the Psych Engine Caching System.
	 */
	public function loadAltMask(path:String)
	{
		var graphic:FlxGraphic = Paths.image(path, null, false);

		if(graphic != null)
			altMaskImage = graphic.bitmap;
	}
