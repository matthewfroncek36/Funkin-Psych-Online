...
import online.backend.wrapper.FlxVideoWrapper;
...
{
	// Variables
	public var bitmap(default, null):FlxVideoWrapper;
	...
	public function play(location:String, shouldLoop:Bool = false):Int
	{
		...
		if (bitmap != null)
		{
			if (FileSystem.exists(Sys.getCwd() + location))
				return (bitmap.play(Sys.getCwd() + location, shouldLoop) ? 0 : -1);
			else
				return (bitmap.play(location, shouldLoop) ? 0 : -1);
		}
		else
			return -1;
	}
