...
class XMLUtil {
	...
	public static function loadSpriteFromXML(spr:FunkinSprite, node:Access, parentFolder:String = "", defaultAnimType:XMLAnimType = BEAT):FunkinSprite {
		...
		spr.loadSprite(Paths.imageAlt('$parentFolder${node.getAtt("sprite").getDefault(spr.name)}', null, true));
		trace(Paths.imageAlt('$parentFolder${node.getAtt("sprite").getDefault(spr.name)}', null, true));
