...
class FlxAnimate extends FlxSprite
{
    ...
    /**
     * Toggles whether the stageMatrix is applied during rendering.
     * Ports the behavior from the LITE fork to scale the root bounds.
     */
    public var applyStageMatrix:Bool = false;

    /**
     * The matrix representing the stage's global transformations.
     */
    public var stageMatrix:FlxMatrix = new FlxMatrix();
    ...
    if( alpha <= 0) return;
        ...
        // Incorporate Stage Matrix at the root before passing it down.
        // This handles calculating both the visual rendering and the _flashRect bounds natively.
        if (applyStageMatrix && stageMatrix != null)
        {
            _matrix.concat(stageMatrix);
        }
    ...
    function drawLimb(limb:FlxFrame, _matrix:FlxMatrix, ?colorTransform:ColorTransform = null, filterin:Bool = false, ?blendMode:BlendMode, ?scrollFactor:FlxPoint = null, cameras:Array<FlxCamera> = null)
    {
        ...
        for (camera in cameras)
        {
            ...
            matrix.identity();
            #if FLX_CNE_FORK
            limb.prepareMatrix(matrix, FlxFrameAngle.ANGLE_0, _checkFlipX() != camera.flipX, _checkFlipY() != camera.flipY);
            #else
            limb.prepareMatrix(matrix, FlxFrameAngle.ANGLE_0, _checkFlipX(), _checkFlipY());
            #end
            ...
            camera.drawPixels(limb, null, matrix, colorTransform, blendMode, (!filterin) ? antialiasing : true, shaderEnabled ? shader : null);
        }
